...
QUESTIONS = [
    {
        ...
        "image": "images.jpg", # File hình ảnh của bạn
...
def show_question_modal(idx):
    ...
    # --- CHÈN HÌNH ẢNH HOẶC VIDEO (NẾU CÓ) ---
    if "image" in q_data:
        try:
            st.image(q_data["image"], use_container_width=True)
        except Exception as e:
            st.error(f"⚠️ Không tìm thấy ảnh. Hãy chắc chắn bạn đã chép file '{q_data['image']}' vào cùng thư mục với code nhé!")
    ...
    if "video" in q_data:
        try:
            ...
            st.error("⚠️ Lỗi không phát được video YouTube.")
        st.markdown("<br>", unsafe_allow_html=True)
    # -----------------------------------------
...
<style>
    ...
    .word-box { display: flex; justify-content: center; align-items: center; height: 110px; background: linear-gradient(145deg, #f44336, #c62828); color: #fffde7; border-radius: 15px; font-size: 42px; font-weight: 900; box-shadow: inset 0px 6px 12px rgba(255,255,255,0.4), 0px 10px 20px rgba(183, 28, 28, 0.5); text-shadow: 2px 2px 6px rgba(0,0,0,0.5); border: 3px solid #ff8a80; margin: 5px; }
    ...
    /* Style nút đáp án và Lồng đèn siêu to */
    div.stButton > button { 
        border-radius: 20px; 
        font-weight: 900; 
        height: auto; 
    ...
    div.stButton > button p { font-size: 30px !important; font-weight: 900 !important; margin: 0 !important; color: #c62828 !important; }
...
for i, b_col in enumerate(btn_cols):
    with b_col:
        # Nút bấm bây giờ nhìn như chiếc lồng đèn
